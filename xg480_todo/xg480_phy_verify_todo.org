* xg480_phy 25g/10g verification todo [90%]
...
* review test plan <2018-07-19 Thu 16:36> [28%]
** TODO the RMON (eth_stats) test [%]
*** TODO termnal loop mode, blocked packet should be counter by eth_stat 
*** TODO pause has own couter in RMON, shoud not count by Mcast. whatever pause action in mac rx
...
** TODO all port have normal + loop traffic (some vlans are loops, some vlans are normal) test.[0%]
   loop + normal + y1564 gen. (if egress + pause + dying gasp gen)
   The PRI is: dyinggasp > pause > normal traffic > loop > y1546 
*** TODO 25G mode in pipe1, 100G mode in pipe0 (ku5p)
